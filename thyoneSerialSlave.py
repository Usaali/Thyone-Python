...
class PrintLines(LineReader):
	...
	def handle_packet(self, data):
		global packetCnt, reqCnfEvent, reqRspEvent, output
	...
	def handle_IND(self,data):
		global packetCnt, msgEvent, byteCnt
		cmd = data[1]
		#print(data.hex())
		if(cmd == 0x73):
			...
			packetCnt+=1
			byteCnt += getPayloadLength(data)-5
			if output:
				src = data[4:8]
				src.reverse()
				rssi = twos(data[8])
...
class SenderThread(Thread):
	...
	def run(self):
		...
		while(True):
			time.sleep(2)
			#msgEvent.wait()
			#self.sendData(sendRequest.CMD_BROADCAST_DATA,self.str2ascii("World"))
			#msgEvent.clear()


ser = serial.serial_for_url('COM3', baudrate=1000000, timeout=1, xonxoff=False, parity = PARITY_EVEN, rtscts = True)
...
with ReaderThread(ser, PrintLines) as protocol:
	...
	while(True):
		if(time.time_ns()-start >= 1e9):
			print("Speed: "+str(byteCnt*8)+"bits/s")
			byteCnt = 0
			start = time.time_ns()
		time.sleep(0.2)
